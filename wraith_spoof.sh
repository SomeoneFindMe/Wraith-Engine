...
# 
# DYNAMIC OUTPUT PATHS (Same as Target APK)
# 
APK_DIR$(dirname "$(realpath "$APK_PATH")")
OUTPUT_DIR"$APK_DIR/output"
...
# 
# PRE-GENERATE ALL VALUES → SAVED TO SDCARD
# 
echo -e "${B}╭─── [ PRE-GENERATING SPOOF VALUES ] ────────────${NC}"
python "$BRAIN" --pregenerate "$APK_COUNT" --append "$HISTORY_JSON" >/dev/null 2>&1
TOTAL_SETS$(python -c "import json; djson.load(open('$HISTORY_JSON')); print(len(d))" 2>/dev/null || echo "?")
echo -e "${B}│${NC} ${G}[✔] Generated $APK_COUNT new sets — Total in history: $TOTAL_SETS${NC}"
echo -e "${B}╰───────────────────────────────────────────────${NC}"
echo ""
...
build_variant() {
    ...
    # Copy base (fast, no re-decompile)
    cp -r "$DECOMPILED_BASE" "$VARIANT_DIR" 2>/dev/null

    # Patch strings using history JSON
...
# Parallel job runner with CPU-aware throttle
declare -A JOB_PIDS
...
# 
# PHASE 4: CLEANUP (base only, keep history)
# 
rm -rf "$DECOMPILED_BASE" 2>/dev/null
...
echo -e "${G}│${NC} ${Y}    APKs  → ${OUTPUT_DIR}/     ${NC}${G}│${NC}"
echo -e "${G}│${NC} ${Y}    Log   → ${HISTORY_JSON} ${NC}${G}│${NC}"
echo -e "${G}╰──────────────────────────────────────────────╯${NC}"
echo ""
